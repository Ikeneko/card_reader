...
clf.connect() でカードを待つ
    ↓ カードを置くと on-connect コールバック（connected()）が呼ばれる
Type3Tag（FeliCa）か確認
    ↓
...
重複読み取りガード（同一カードが duplicate_guard_seconds 以内なら無視）
    ↓
未登録なら登録ダイアログを表示して氏名を確定する（カードを置いたまま待機）
    ↓ 氏名確定後
学籍番号・氏名を GUI に表示
    ↓
log_data の最後のセッションに exit がなければ入室、あれば退室と判定
    ↓
GUI ステータス更新（入室：緑 / 退室：青） / 効果音 / Slack 通知 / ログ保存
    ↓
カードを離す → connected() が return True で返る → clf.connect() が返る
    ↓
次のカードを待つ（ループ先頭へ）
```

**すべての処理は `connected()` 内で完結する：**<br>
登録ダイアログ・GUI 更新・入退室判定・効果音・Slack 通知・ログ保存はカードが乗っている間に `connected()` 内でまとめて実行されます。`clf.connect()` が返った後の後続処理はありません。
